...
class gaNdalFClassifier(nn.Module):
    ...
    def init_dataset(self):
        galaxies = DESGalaxies(cfg=self.cfg, kind="classifier_training")
    ...
    def run_training(self):
        ...
        df_test_data = pd.DataFrame(
            np.concatenate((input_data, arr_flow_output), axis=1), columns=self.cfg["INPUT_COLS_MAG_CLASSF"] + self.cfg["OUTPUT_COLS_MAG_FLOW"]
        )
        ...
        if self.cfg["APPLY_YJ_TRANSFORM_CLASSF"] is True:
            ...
            df_test_data = self.galaxies.yj_inverse_transform_data(data_frame=df_test_data, columns=trans_col)
